...
```{r}
# Create a new column for plotting: combines Species and Site for x-axis
combined_data <- filtered_data %>%
  filter(Species != "NA") %>%
  filter(!Species %in% c("P. effusa", "P. acuta")) %>%
  filter(Site %in% c("S4", "S14")) %>%  # Replace with actual sites
  mutate(Species_Site = paste(Species, Site, sep = "_"))

# Plot: use Species_Site as x-axis and color by Species
bulk_density_combined_plot <- combined_data %>%
  ggplot(aes(x = Species_Site, y = Bulk.Density, color = Species)) +
  geom_point(
    size = 2,
    alpha = 0.3
  ) +
...
combined_data <- combined_data %>%
  mutate(
    Species_Site = paste0(Site, "\n", Species)  # Line break for neatness
  )
