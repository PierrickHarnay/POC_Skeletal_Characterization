...
bulk.density.plot.outlier.removed.species <- filtered_data %>%
  ggplot(aes(x = Site, y = Bulk.Density, color = Species), group=Species) +  # Add 'color = Species' to map colors to the Species variable
  geom_point(position = position_dodge(width = 0.9), size = 1, alpha=0.6) +
  scale_color_manual(values = species_colors) +  # Apply the manual color scale
  #facet_wrap(~ Species) +  # Facet by Species
  theme_classic()+
  labs(title="Outlier Removed",
